...
namespace GoodJob.Wax.Controllers.Meshing
{
    public class MeshController : MonoBehaviour
    {
        ...
        public float BuildedTriangles, BuildedTrianglesThreshold;
        ...
        private void BuildMesh()
        {
            for (int i = 0; i < _waxVerticesWorld.Length; i++)
            {
                ...
                {
                    if (_newTrianglesArray[triangleList[j]] == _waxTriangles[triangleList[j]]) continue;
                    _newTrianglesArray[triangleList[j]] = _waxTriangles[triangleList[j]];
                    BuildedTriangles++;
                }
            }

            if (_newMesh.triangles == _newTrianglesArray) return;
            _newMesh.triangles = _newTrianglesArray;
